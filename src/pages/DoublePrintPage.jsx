...
export default function DoublePrintPage({ isDarkMode, onBack }) {
  const [photo, setPhoto] = useState(null);
  const [isUploading, setIsUploading] = useState(false);

  // Layout Controls
  const [orientation, setOrientation] = useState(() => localStorage.getItem('dp_orientation') || 'landscape'); // 'portrait' | 'landscape'
  const [splitMode, setSplitMode] = useState(() => localStorage.getItem('dp_splitMode') || 'vertical'); // 'vertical' | 'horizontal'
  ...
  useEffect(() => {
    ...
    localStorage.setItem('dp_orientation', orientation);
    localStorage.setItem('dp_splitMode', splitMode);
    localStorage.setItem('dp_imageZoom', imageZoom.toString());
    localStorage.setItem('dp_imageOffsetX', imageOffsetX.toString());
    localStorage.setItem('dp_imageOffsetY', imageOffsetY.toString());

    if (!fontFamily.startsWith('CustomFont_')) {
      localStorage.setItem('dp_fontFamily', fontFamily);
    }
  }, [printerProfile, customNames, customDate, fontFamily, showDashedLine, fontWeight, fontSizeScale, imageMargin, textAreaHeight, textMargin, orientation, splitMode, imageZoom, imageOffsetX, imageOffsetY]);

  const [imageZoom, setImageZoom] = useState(() => Number(localStorage.getItem('dp_imageZoom')) || 80);
  const [imageOffsetX, setImageOffsetX] = useState(() => Number(localStorage.getItem('dp_imageOffsetX')) || 0);
  const [imageOffsetY, setImageOffsetY] = useState(() => Number(localStorage.getItem('dp_imageOffsetY')) || 0);
  ...
  const executePrint = () => {
    ...
    if (!printWindow) {
      alert("Vui lòng cho phép mở Popup để tính năng in hoạt động.");
      return;
    }
    const writeToPrintWindow = (imgSrc, isLandscape) => {
      printWindow.document.write(`
        <html><head><title>SOJI Studio - Print</title>
        <style>@page { margin: 0; size: ${isLandscape ? '6in 4in' : '4in 6in'}; }
        ...
        img { max-width: calc(100% - 0.3in); max-height: calc(100% - 0.3in); object-fit: contain; }</style>
        </head><body><img src="${imgSrc}" onload="setTimeout(() => { window.print(); window.close(); }, 200);" /></body></html>
      `);
    ...
    if (needsRotation) {
      ...
      img.onload = () => {
        ...
        offCanvas.width = img.height; offCanvas.height = img.width;
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.download = `double-print-${Date.now()}.jpg`;
  ...
  const renderLayoutPanel = () => (
    <div className="settings-section">
      ...
      <div className={`collapsible-content ${isPrinterExpanded ? 'expanded' : 'collapsed'}`}>
        ...
        <div className="template-options">
          <button className={`template-btn ${orientation === 'portrait' ? 'active' : ''}`} onClick={() => setOrientation('portrait')}>Portrait (4x6)</button>
          <button className={`template-btn ${orientation === 'landscape' ? 'active' : ''}`} onClick={() => setOrientation('landscape')}>Landscape (6x4)</button>
        </div>
    ...
    <div className="settings-section">
      ...
      <div className={`collapsible-content ${isTypographyExpanded ? 'expanded' : 'collapsed'}`}>
        <div className="yearbook-pill-row">
          {FONT_OPTIONS.map((f) => (
            <button key={f.id} className={`yearbook-pill ${fontFamily === f.id ? 'active' : ''}`} onClick={() => setFontFamily(f.id)}>
              {f.label}
            </button>
          ))}
        </div>
        ...
        <div className="control-group" style={{ marginTop: '0.75rem' }}>
          ...
          <input type="range" min="50" max="200" value={fontSizeScale} onChange={(e) => setFontSizeScale(Number(e.target.value))} className="slider" />
        </div>
        <div className="control-group" style={{ marginTop: '1rem' }}>
          <button className="upload-logo-btn" onClick={() => document.getElementById('dp-font-upload').click()}>Upload Custom Font</button>
          <input id="dp-font-upload" type="file" accept=".ttf,.otf,.woff" style={{ display: 'none' }} onChange={handleFontUpload} />
        </div>
        <h3 className="template-heading" style={{ marginTop: '1.5rem', cursor: 'default' }}>Options</h3>
  ...
  const renderImageSettings = () => (
    <div className="settings-section">
      <h3 className="template-heading" onClick={() => setIsFitExpanded(!isFitExpanded)} style={{ cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span>Image Fit & Pan</span>
        {isFitExpanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
      </h3>
      <div className={`collapsible-content ${isFitExpanded ? 'expanded' : 'collapsed'}`}>
        <div className="control-group">
        ...
        <div className="control-group" style={{ marginTop: '0.75rem' }}>
          <label className="control-label">Zoom: {imageZoom}%</label>
          <input type="range" min="50" max="200" value={imageZoom} onChange={(e) => setImageZoom(Number(e.target.value))} className="slider" />
        </div>
        <div className="control-group" style={{ marginTop: '0.75rem' }}>
        ...
        <div className="control-group" style={{ marginTop: '0.75rem' }}>
          <label className="control-label">Pan X: {imageOffsetX}%</label>
          <input type="range" min="-100" max="100" value={imageOffsetX} onChange={(e) => setImageOffsetX(Number(e.target.value))} className="slider" />
        </div>
        <div className="control-group" style={{ marginTop: '0.75rem' }}>
          <label className="control-label">Pan Y: {imageOffsetY}%</label>
          <input type="range" min="-100" max="100" value={imageOffsetY} onChange={(e) => setImageOffsetY(Number(e.target.value))} className="slider" />
        </div>
        <button className="template-btn" style={{ marginTop: '1rem', width: '100%', justifyContent: 'center' }} onClick={() => { setImageOffsetX(0); setImageOffsetY(0); }}>Reset Pan</button>
      </div>
    </div>
  );

  const UploadZone = () => (
    <div className="yearbook-upload-zone" style={{ position: 'relative' }} onDragOver={(e) => e.preventDefault()} onDrop={handleDrop} onClick={() => document.getElementById('dp-file-input').click()}>
      {onBack && (
        <button className="app-tab-btn active" style={{ position: 'absolute', top: '2rem', left: '2rem', gap: '0.5rem', zIndex: 10 }} onClick={(e) => { e.stopPropagation(); onBack(); }}>
          <ArrowLeft size={16} /> Layout Menu
        </button>
      )}
      <input id="dp-file-input" type="file" accept="image/jpeg,image/png,image/heic" style={{ display: 'none' }} onChange={(e) => handleFilesInput(e.target.files)} />
      {isUploading ? (
        <><div className="spinner" /><p>Optimizing image...</p></>
      ) : (
        <><Printer size={52} strokeWidth={1.2} className="yearbook-upload-icon" />
          <h2 className="yearbook-upload-title">Double Print (4x6)</h2>
          <p className="yearbook-upload-desc">Upload a photo to automatically duplicate<br /><span>and layout for 4x6 printing</span></p></>
      )}
    </div>
  );
      ...
      {!photo ? (
        ...
        <div className="yearbook-layout-wrapper">
              ...
              <div className="yearbook-sidebar-photos" style={{ marginBottom: '1.5rem' }}>
                ...
                <div className="thumbnail-strip" style={{ maxHeight: '180px' }}>
                  <div className="thumbnail-item active">
                    ...
                    <button className="remove-photo-btn" onClick={handleReset} title="Remove"><X size={14} /></button>
                  </div>
                </div>
                {renderImageSettings()}
              </div>
          ...
          <section className="yearbook-canvas-section print-canvas-section">
            <DoublePrintCanvas
              photo={photo} orientation={orientation} splitMode={splitMode}
              ...
              onPanChange={handlePanChange} onZoomChange={handleZoomChange}
            ...
            <div className="panel-box">
              <div className="yearbook-sidebar-controls" style={{ flex: 1, maxHeight: 'none', overflowY: 'visible' }}>
                ...
                <div className="settings-section" style={{ marginTop: '1.5rem', display: 'flex', gap: '0.5rem', flexDirection: 'column' }}>
                  ...
                  <button className="download-btn" onClick={handleDownload} disabled={isDownloading} style={{ opacity: isDownloading ? 0.7 : 1, width: '100%', margin: 0 }}>
