...
export default function WeddingFramePage({ isDarkMode, onBack }) {
  const [photo, setPhoto] = useState(null);
  const [isUploading, setIsUploading] = useState(false);

  // Layout Controls
  const [orientation, setOrientation] = useState(() => localStorage.getItem('wf_orientation') || 'landscape'); // 'portrait' | 'landscape'
  ...
  const [textAreaHeight, setTextAreaHeight] = useState(() => {
    const v = localStorage.getItem('wf_textAreaHeight');
    return v !== null ? Number(v) : 23;
  });
  const [textMargin, setTextMargin] = useState(() => {
    const v = localStorage.getItem('wf_textMargin');
  ...
  useEffect(() => {
    ...
    localStorage.setItem('wf_textAreaHeight', textAreaHeight.toString());
    localStorage.setItem('wf_textMargin', textMargin.toString());
    localStorage.setItem('wf_orientation', orientation);
    localStorage.setItem('wf_imageZoom', imageZoom.toString());
    localStorage.setItem('wf_imageOffsetX', imageOffsetX.toString());
    localStorage.setItem('wf_imageOffsetY', imageOffsetY.toString());

    if (!fontFamily.startsWith('CustomFont_')) {
      localStorage.setItem('wf_fontFamily', fontFamily);
    }
  }, [printerProfile, customNames, customDate, fontFamily, fontWeight, fontSizeScale, imageMargin, textAreaHeight, textMargin, orientation, imageZoom, imageOffsetX, imageOffsetY]);

  const [imageZoom, setImageZoom] = useState(() => Number(localStorage.getItem('wf_imageZoom')) || 80);
  const [imageOffsetX, setImageOffsetX] = useState(() => Number(localStorage.getItem('wf_imageOffsetX')) || 0);
  const [imageOffsetY, setImageOffsetY] = useState(() => Number(localStorage.getItem('wf_imageOffsetY')) || 0);
  ...
  const handleFilesInput = useCallback(async (files) => {
    ...
    try {
      ...
      img.onload = () => {
        ...
        setOrientation(isLandscapePhoto ? 'landscape' : 'portrait');
      };
      img.src = displayUrl;
    } finally {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.download = `wedding-frame-${Date.now()}.jpg`;
  ...
  const renderStylePanel = () => (
    <div className="settings-section">
      ...
      <div className={`collapsible-content ${isTypographyExpanded ? 'expanded' : 'collapsed'}`}>
        ...
        <div className="control-group" style={{ marginTop: '1rem' }}>
          <button className="upload-logo-btn" onClick={() => document.getElementById('wf-font-upload').click()}>Upload Custom Font</button>
          <input id="wf-font-upload" type="file" accept=".ttf,.otf,.woff" style={{ display: 'none' }} onChange={handleFontUpload} />
          {fontFamily.startsWith('CustomFont_') && <p className="upload-hint" style={{ marginTop: '0.25rem' }}>Custom font loaded.</p>}
        </div>
      </div>
    </div>
  );
  ...
  const UploadZone = () => (
    <div className="yearbook-upload-zone" style={{ position: 'relative' }} onDragOver={(e) => e.preventDefault()} onDrop={handleDrop} onClick={() => document.getElementById('wf-file-input').click()}>
      {onBack && (
      ...
      <input id="wf-file-input" type="file" accept="image/jpeg,image/png,image/heic" style={{ display: 'none' }} onChange={(e) => handleFilesInput(e.target.files)} />
      {isUploading ? (
        ...
        <><Printer size={52} strokeWidth={1.2} className="yearbook-upload-icon" />
          ...
          <p className="yearbook-upload-desc">Upload a photo to create<br /><span>a 4x6 wedding layout</span></p></>
      )}
    </div>
  );
